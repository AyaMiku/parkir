...
if (!JWT_SECRET) {
  console.error(
    "❌ ERROR: JWT_SECRET tidak ditemukan dalam environment variables!",
  );
...
const verifyUser = async (req, res, next) => {
  try {
    ...
    console.log(
      `✅ User Authenticated: ID = ${req.user.id}, Role = ${req.user.role}`,
    );
...
const isAdmin = (req, res, next) => {
  console.log(
    `🔍 Checking role for User ID: ${req.user?.id}, Role: ${req.user?.role}`,
  );
  ...
  if (!req.user || !req.user.id || !req.user.role) {
    ...
    return res
      .status(401)
      .json({ message: "Autentikasi gagal. Silakan login kembali." });
  }
  ...
  if (req.user.role !== "admin") {
    ...
    return res
      .status(403)
      .json({ message: "Akses ditolak! Hanya Admin yang dapat masuk." });
  }

  console.log("✅ Akses Admin diizinkan!");
  next();
};
...
const isUser = (req, res, next) => {
  ...
  if (req.user.role !== "user") {
    ...
    return res
      .status(403)
      .json({ message: "Akses ditolak! Hanya User yang dapat masuk." });
  }
